...
#define ThrottlePinCheck A1 // Throttle Pin on A1
...
void setup() {
  ...
  pinMode(ThrottlePinCheck, INPUT);
...
void loop() {
  ...
  Throttle_Value =
      map(BikerRpmFiltered, 0, BikerMaxRpm, MinThrottleValue, MaxThrottleValue);
  ...
  Serial.print(analogRead(ThrottlePinCheck) * ANALOG_TO_VOLT_IN);
  Serial.println();
