...
void check_direction() {
  if (digitalRead(PIN_DIRECTION_IN) == HIGH) {
    if (!moving_forward) {
      start_moving();
    }
  } else {
...
void loop() {
  float torque = analogRead(PIN_TORQUE_IN) * ANALOG_TO_VOLT_IN;
  int speed = digitalRead(PIN_SPEED_IN);
  int direction = digitalRead(PIN_DIRECTION_IN);
